...
export const CloseIcon = ({ handleClick, size, id }) => {
    const styles = {
        close: {
    ...
    return (
        <span style={styles.close} id={id} onClick={(event) => {
            const val = event.target.id || event.target.parentNode.id;
            handleClick(val);
        }}>
            <Img style={{width: iconSize, height: iconSize}}
